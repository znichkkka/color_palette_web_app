...
from http_utils import create_response, redirect, json_response, json_error
...
def show_result_page(request):
    try:
        ...
        if image_id is None:
            return create_error_response(400, "Bad Request", "Некорректный id результата")

        image = get_image(image_id)

        if image is None:
            return create_error_response(404, "Not Found", "Результат не найден")

        access_response = check_image_access_for_page(user, image)
        ...
        return create_error_response(500, "Internal Server Error", "Ошибка при открытии результата")
...
def register_user(request):
    try:
        ...
        return create_error_response(500, "Internal Server Error", "Ошибка при регистрации пользователя")
...
def login_user(request):
    try:
        ...
        return create_error_response(500, "Internal Server Error", "Ошибка при входе в аккаунт")
...
def create_image_api(request):
    try:
        ...
        if "multipart/form-data" not in content_type:
            return json_error(415, "Unsupported Media Type", "Форма отправлена в неверном формате")

        form = parse_multipart_form(request.body, content_type)
        uploaded_file = form["files"].get("image")

        if uploaded_file is None:
            return json_error(400, "Bad Request", "Файл не был загружен")

        filename = uploaded_file["filename"]
        file_data = uploaded_file["content"]
        colors_count = get_colors_count(form)

        error_response = validate_uploaded_image(filename, file_data)
        ...
        if not is_image_file(filepath):
            ...
            return json_error(415, "Unsupported Media Type", "Загруженный файл не является изображением")
        ...
        return json_response(
            ...
            "Created",
            {
                ...
                "image": {
                    ...
                    "palette": create_palette_data(palette),
                    "result_url": f"/result?id={image_id}",
                    "palette_url": f"/api/palette?id={image_id}"
                }
            },
            {
                "Location": f"/api/images?id={image_id}"
            }
        )

    except Exception as error:
        log_error("Ошибка загрузки изображения через API: " + str(error))
        return json_error(500, "Internal Server Error", "Ошибка при обработке изображения")
...
def get_images_api(request):
    try:
        ...
        if image_id is not None:
            return get_one_image_api(user, image_id)

        if user is None:
            return json_error(401, "Unauthorized", "Для получения истории необходимо войти в аккаунт")

        images = get_all_images(user["id"])
        result = []

        for image in images:
            result.append(create_image_api_dict(image))
        ...
        log_error("Ошибка получения изображений через API: " + str(error))
        return json_error(500, "Internal Server Error", "Ошибка при получении данных")


def get_one_image_api(user, image_id):
    image = get_image(image_id)

    if image is None:
        return json_error(404, "Not Found", "Результат не найден")

    access_response = check_image_access_for_api(user, image)
    ...
    return json_response(
        ...
        {
            "success": True,
            "image": create_image_api_dict(image)
        }
    )
...
def delete_image_api(request):
    try:
        ...
        if user is None:
            return json_error(401, "Unauthorized", "Для удаления изображения необходимо войти в аккаунт")

        image_id = get_id_from_query(request, "id")

        if image_id is None:
            return json_error(400, "Bad Request", "Некорректный id изображения")

        filepath = delete_image(image_id, user["id"])

        if filepath is None:
            return json_error(404, "Not Found", "Изображение не найдено")
        ...
        log_error("Ошибка удаления изображения через API: " + str(error))
        return json_error(500, "Internal Server Error", "Ошибка при удалении записи")
...
def clear_history_api(request):
    try:
        ...
        if user is None:
            return json_error(401, "Unauthorized", "Для очистки истории необходимо войти в аккаунт")

        filepaths = delete_all_user_images(user["id"])
        ...
        return json_response(
            ...
            {
                "success": True,
                "message": "История загрузок очищена"
            }
        )

    except Exception as error:
        log_error("Ошибка очистки истории через API: " + str(error))
        return json_error(500, "Internal Server Error", "Ошибка при очистке истории")
...
def delete_account_api(request):
    try:
        ...
        if user is None:
            return json_error(401, "Unauthorized", "Для удаления аккаунта необходимо войти в аккаунт")
        ...
        log_error("Ошибка удаления аккаунта через API: " + str(error))
        return json_error(500, "Internal Server Error", "Ошибка при удалении аккаунта")


def download_palette_api(request):
    try:
        user = get_current_user(request)
        image_id = get_id_from_query(request, "id")

        if image_id is None:
            return json_error(400, "Bad Request", "Некорректный id палитры")

        image = get_image(image_id)

        if image is None:
            return json_error(404, "Not Found", "Палитра не найдена")
        ...
        log_error("Ошибка скачивания палитры через API: " + str(error))
        return json_error(500, "Internal Server Error", "Ошибка при создании изображения палитры")


def validate_uploaded_image(filename, file_data):
    if filename == "":
        return json_error(400, "Bad Request", "Имя файла пустое")

    if len(file_data) > MAX_FILE_SIZE:
        return json_error(413, "Payload Too Large", "Размер файла не должен превышать 20 МБ")

    if not has_allowed_image_extension(filename):
        return json_error(415, "Unsupported Media Type", "Недопустимый формат файла")
...
def check_image_access_for_page(user, image):
...
def check_image_access_for_api(user, image):
    if image["user_id"] is None:
        return None
...
def create_palette_data(palette):
    ...
    for color in palette:
        result.append({
            "hex": color,
            "rgb": hex_to_rgb_text(color),
            "hsl": hex_to_hsl_text(color)
        })

    return result


def create_image_api_dict(image):
    return {
        "id": image["id"],
        "filename": image["filename"],
        "url": get_image_url(image["filepath"]),
        "upload_date": image["upload_date"],
        "colors_count": image["colors_count"],
        "palette": create_palette_data(image["palette"]),
        "result_url": f"/result?id={image['id']}",
        "palette_url": f"/api/palette?id={image['id']}"
    }
